...
func TestUseCase_SendDevicesToAgent(t *testing.T) {
		...
		// =======================================
		{
			...
			mocks: mocks{
				requestSvReceiveContent: &requestSvReceiveContent{
					...
					funcName: shared.TargetGetRooms,
				},
				responseSvReceiveContent: &responseSvReceiveContent{
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := uc.SendDevicesToAgent(tt.args.agentID); (err != nil) != tt.wantErr {
				t.Errorf("UseCase.SendDevicesToAgent() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
